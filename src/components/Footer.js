...
const Footer = () => {
  ...
  // Hook para detectar si el footer está en vista
  const { ref, inView } = useInView({
    triggerOnce: true, // solo se activa la animación una vez
    threshold: 0.2,    // el 20% del footer debe estar en vista para activarse
  });

  return (
    <motion.div
      ref={ref}  // Referenciamos el contenedor del footer
      initial={{ opacity: 0, y: 100 }}  // Estado inicial de la animación
      animate={{ opacity: inView ? 1 : 0, y: inView ? 0 : 100 }}  // Animación de entrada
